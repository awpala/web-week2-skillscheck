...
function lovesCodeChecker(x) {
	return x; // N.B. can also use if-else or ternary operator, but this is most terse option for this specific use case
}
...
const me = {
	...
	state: 'IL',
	age: 30,
	greeter: () => `Hello! My name is ${firstName} and I live in ${IL}`
}
...
function bigOrSmall(arr) {
	...
	answers = arr.map(el => (el > 100) ? 'big' : 'small');
...
// Code Here
const total = myNumbers.reduce((acc, val) => acc + val, 0);
...
// Code Here
const totallyAwesome = people.reduce((acc, val) => acc + val.awesomeLevel, 0);